...
jsZIP.file(`${data.filename}.xml`, xmlFirmado);

const bufferZIP = await jsZIP.generateAsync({type: 'nodebuffer'});

await fs.writeFile(
    path.join(process.cwd(), `${data.filename}.zip`),
    bufferZIP
);

const _http = new RazorbladeHttp('development');
const cdrBase64 = await _http.sendBill(path.join(process.cwd(), `${data.filename}.zip`), data.filename);

await fs.writeFile(
    path.join(process.cwd(),"cdr", `R-${data.filename}.zip`),
